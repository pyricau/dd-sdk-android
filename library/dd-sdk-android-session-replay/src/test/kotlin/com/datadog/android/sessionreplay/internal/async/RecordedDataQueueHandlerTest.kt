...
internal class RecordedDataQueueHandlerTest {
    ...
    fun `M snapshot item contains correct fields W add() { valid RumContextData }`() {
        ...
        // Then
        assertThat(item!!.prevRumContext).isEqualTo(currentRumContextData?.prevRumContext)
        assertThat(item.newRumContext).isEqualTo(currentRumContextData?.newRumContext)
        assertThat(item.timestamp).isEqualTo(currentRumContextData?.timestamp)
    ...
    fun `M consume items in the correct order W tryToConsumeItems() { spawn multiple threads }`() {
        // Given
        val item1 = createFakeSnapshotItemWithDelay(1)
        val item2 = createFakeSnapshotItemWithDelay(2)
        val item3 = createFakeSnapshotItemWithDelay(3)

        assertThat(testedHandler.recordedDataQueue.size).isEqualTo(3)
        val itemTimestamp = item1.timestamp
    ...
    private fun createFakeSnapshotItemWithDelay(delay: Int): SnapshotRecordedDataQueueItem {
